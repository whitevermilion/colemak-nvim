-- .config/nvim/lua/passiveplugins/heirline/components.lua
...
local function OverseerTasksForStatus(st)
  return {
    condition = function(self) return self.tasks[st] end,
    provider = function(self) return string.format("%s%d", self.symbols[st], #self.tasks[st]) end,
    hl = function(_)
      return {
...
M.Mode = {
  ...
  update = {
    "ModeChanged",
    pattern = "*:*",
    callback = vim.schedule_wrap(function() pcall(vim.cmd, "redrawstatus") end),
  },
}
...
M.CodeiumStatus = {
  ...
  provider = function(self)
    if not self.codeium_exist then return "" end
...
M.Git = {
  ...
  hl = function(self) return { fg = self.has_changes and palette.maroon or dim_color } end,
...
M.FileIcon = {
  condition = function(self) return vim.fn.fnamemodify(self.filename, ":.") ~= "" end,
  init = function(self)
    ...
    if bt and bt == "terminal" then icon = "" end
    self.icon = icon
    self.icon_color = string.format("#%06x", vim.api.nvim_get_hl(0, { name = hl })["fg"])
  end,
  provider = function(self) return self.icon and (self.icon .. " ") end,
  hl = function(self) return { fg = self.is_modified and self.icon_color or dim_color } end,
}
...
M.FilePath = {
  provider = function(self)
    ...
    if filename == "" then return vim.bo.filetype ~= "" and vim.bo.filetype or vim.bo.buftype end
...
M.FileFlags = {
  {
    ...
    condition = function(self)
      ...
      if vim.tbl_contains(ignored_filetypes, ft) then result = false end
      return result
    end,
    provider = " 􀴥 ",
    hl = function(self) return { fg = self.icon_color, bold = self.is_active } end,
  },
  {
    condition = function(self)
...
M.SimpleIndicator = {
  condition = function() return vim.g.simple_indicator_on end,
  hl = { fg = palette.sky },
  provider = "",
}
